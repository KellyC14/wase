...
/**
 * 
 * This script allows an administrator to get a list of owners/managers who have blocks in the system
...
$users = WaseSQL::doQuery('SELECT DISTINCT userid FROM ' . WaseUtil::getParm('DATABASE') . '.WaseBlock WHERE date(startdatetime) >= ' . WaseSQL::sqlSafe($date));
...
<h1><?php echo 'WASE Active Owners'; if ($institution = $_SERVER['INSTITUTION']) echo ' for "' . $institution . '"';?></h1>
<?php if (!$ldap) {
    $note = "Note: invoke this script with a 'date=yyyy-mm-dd' argument to change the minimum block start date"; ?>
    <h2>LDAP not supported; using  <?php 
    }
    else {
    $note = "Note: invoke this script with an 'attr=value' GET argument to change the classifying attribute, and/or 'date=yyyy-mm-dd' to change the minimum block start date'"?>
    <h2>Using '<?php echo $attr?>' as classifying attribute, 
    <?php }
     echo $date?> as the minimum block start date.</h2>
     <?php echo $note;?>
.<br /><br />
...
?>
<h3><?php echo $count;?> active owner(s)</h3>
</table>
</body>
</html>