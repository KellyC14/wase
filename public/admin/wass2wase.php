...
// Read in optional calendar id variable, and test for ALL.
if ($calid = $_REQUEST['calid']) {
    if ($calid == 'ALL') {
        $calid = '';
        $where = ' WHERE 1 ';
    } else
        $where = ' WHERE calendarid=' . WaseSQL::sqlSafe($calid);
} else
    die('You must provide the id of the wass calendar you wish to migrate, or the word ALL, as an HTTP calid= query parameter.');
...
/* Init counters */
$cals = 0; $series = 0; $periods = 0; $blocks = 0;  $appts = 0; $managers = 0;  $members = 0;
...
/* Build a list of all wass calendars. */
$wasscals = WaseSQL::doQuery('SELECT * FROM ' . $_SESSION['ALT_DATABASE'] . '.wassCalendar ' . $where . ';');
