...
switch ($action) {
    ...
    case 'GETAPP':
        ...
        try {
            ...
            WaseUtil::Error(14, array(
                'Unable to locate specified ' . WaseUtil::getParm('APPOINTMENT')  . '.'
            ), 'HTMLFORMAT,DIE');
        }
    ...
    case 'DELAPP':
		...
		/* Return an iCalendar stream to delete an appointment */
        ...
        if (! $app->isViewable($_SESSION['authtype'], $_SESSION['authid']))
            ...
            /* Now build an iCalendar stream to delete the appointment */
    ...
    case 'MAILAPP':
        ...
        /* Send the email */
        WaseUtil::Mailer($toid, $block->APPTHING . ' scheduled: ' . $app->name . ' with ' . $block->name . ' at ' . WaseUtil::AmPm($starttime) . ' on ' . WaseUtil::usDate($startdate), $icalstream, $icalheaders);
